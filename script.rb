...
def get_team_stats(team_id)
  response = HTTParty.get("https://statsapi.web.nhl.com/api/v1/teams/#{team_id}?expand=team.stats")
  response.parsed_response['teams'].each do |team|
...
def get_fan_team_stats_array(fan_team_hash)
  ...
  team_stats_array = []
  team_ids.each do |team_id|
    response = HTTParty.get("https://statsapi.web.nhl.com/api/v1/teams/#{team_id}?expand=team.stats&expand=team.schedule.next")
    response.parsed_response['teams'].each do |team|
      ...
      if team.key?('nextGameSchedule') && !team['nextGameSchedule'].nil? && team['nextGameSchedule'].key?('dates') && !team['nextGameSchedule']['dates'].empty?
        team_stats_array.last['nextGameDate'] =
          team['nextGameSchedule']['dates'][0]['date']

        team_stats_array.last['nextGameDate'] =
          team['nextGameSchedule']['dates'][0]['date']
        team_stats_array.last['nextGameOpponent'] =
          team['nextGameSchedule']['dates'][0]['games'][0]['teams']['away']['team']['name']
        ...
        if team_stats_array.last['nextGameOpponent'].include? fan_team_hash[team_stats_array.last['fan']]
          team_stats_array.last['nextGameOpponent'] =
            team['nextGameSchedule']['dates'][0]['games'][0]['teams']['home']['team']['name']
        end
      else
      ...
      response.parsed_response['records'].each do |record|
        next unless record['teamRecords'].any? { |h| h['team']['id'] == team_id }

        team_stats_array.last['leagueRank'] = record['teamRecords'].find do |h|
          h['team']['id'] == team_id
        end['leagueRank']
        # team_stats_array.last['conferenceName'] = record['conference']['name']
        team_stats_array.last['conferenceRank'] = record['teamRecords'].find do |h|
          h['team']['id'] == team_id
        end['conferenceRank']
        # team_stats_array.last['divisionName'] = record['division']['name']
        team_stats_array.last['divisionRank'] = record['teamRecords'].find do |h|
          h['team']['id'] == team_id
        end['divisionRank']
        team_stats_array.last['wildCardRank'] = record['teamRecords'].find do |h|
          h['team']['id'] == team_id
        end['wildCardRank']
        team_stats_array.last['streakCode'] = record['teamRecords'].find do |h|
          h['team']['id'] == team_id
        end['streak']['streakCode']
        # Lookup the conference name and division name from the team id separately and add to the team stats hash
        response = HTTParty.get("https://statsapi.web.nhl.com/api/v1/teams/#{team_id}?expand=team.stats")
        team_stats_array.last['conferenceName'] =
          response.parsed_response['teams'][0]['conference']['name']
        team_stats_array.last['divisionName'] =
          response.parsed_response['teams'][0]['division']['name']
      end
    end
  end
  team_stats_array
...
def sort_fan_team_stats_by_wins(fan_team_hash)
  ...
  team_stats_array.sort_by { |hsh| hsh['wins'] }.reverse
end
...
def sort_fan_team_stats_by_league_rank(fan_team_hash)
  ...
  sorted_team_stats_array = sorted_team_stats_array.sort_by do |hsh|
    hsh['conferenceRank'].to_i
  end
  sorted_team_stats_array = sorted_team_stats_array.sort_by { |hsh| hsh['divisionRank'].to_i }
  sorted_team_stats_array.sort_by { |hsh| hsh['wildCardRank'].to_i }
end
...
def get_playoff_standings(season)
  response = HTTParty.get("https://statsapi.web.nhl.com/api/v1/tournaments/playoffs/?expand=round.series,schedule.game.seriesSummary&season=#{season}")
...
puts "File created at: #{File.absolute_path('_site/index.html')}" if File.file?('_site/index.html')
...
def generate_series_html(series)
  ...
  if series['currentGame']['seriesSummary']['gameTime']
    series['currentGame']['seriesSummary']['gameTime'] =
      tz.utc_to_local(Time.parse(series['currentGame']['seriesSummary']['gameTime'])).strftime('%A, %b %d %Y %I:%M %p')
  end
...
def generate_playoff_page(current_season, playoff_standings)
  ...
  rounds_section = if playoff_standings && !playoff_standings['rounds'].nil?
                     playoff_standings['rounds'].map do |round|
                       generate_round_html(round)
                     end.join("\n")
                   else
                     "<h1 class='color-fg-success'>Playoffs have not started!</h1>"
                   end

  File.write('_site/playoffs.html', <<~HTML)
    <!DOCTYPE html>
    <html lang='en'>
    <head>
      <meta charset='UTF-8'>
