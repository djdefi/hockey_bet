...
(function() {
  ...
  const KeyboardShortcuts = {
    shortcuts: {
      'l': { action: () => switchTab('league'), description: 'View League tab' },
      'm': { action: () => switchTab('matchups'), description: 'View Matchups tab' },
      's': { action: () => switchTab('standings'), description: 'View Standings tab' },
      't': { action: () => switchTab('trends'), description: 'View Trends tab' },
  ...
  const FocusManager = {
    ...
    enhanceFocusIndicators: function() {
      ...
      style.textContent = `
        *:focus {
          outline: 3px solid rgba(33, 209, 159, 0.8) !important;
