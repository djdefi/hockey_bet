...
(function() {
  ...
  const SocialReactions = {
    ...
    init: function() {
      this.loadReactions();
      // Disabled until properly implemented - placeholders were confusing
    ...
    saveReactions: function() {
      ...
      try {
        ...
        if (data.length > 4 * 1024 * 1024) { // 4MB soft limit
          ...
          // Keep only recent entries (simple strategy)
        ...
        if (e.name === 'QuotaExceededError') {
          ...
          // Try to clear and retry with pruned data
          this.reactions = {};
    ...
    initReactionButtons: function() {
      // Add reaction buttons only to larger cards (not stat-card-square - they're too small at 125x125px)
      ...
      cards.forEach((card, index) => {
        ...
        // Style the reaction bar with minimal spacing
    ...
    showEmojiPicker: function(cardId, button) {
      ...
      // Centralized cleanup to avoid leaking timeouts and event listeners
      const cleanup = () => {
      ...
      const timeoutId = setTimeout(() => {
        const closeHandler = (e) => {
          if (!picker.contains(e.target) && e.target !== button) {
            // Ensure we always clean up listeners and timeouts when closing
            cleanup();
            picker.remove();
          }
        };
      ...
      // Store timeout id so it can be cancelled if the picker is removed early
  ...
  const QuickCompare = {
    ...
    addCompareButtons: function() {
      // Add compare buttons only to larger cards (not stat-card-square - they're too small)
      const teamCards = document.querySelectorAll('.matchup-card, .team-card');
      ...
      teamCards.forEach(card => {
        ...
        compareBtn.innerHTML = '<iconify-icon icon="solar:sort-horizontal-bold" width="14" height="14"></iconify-icon> Compare';
    ...
    showCompareModal: function(card) {
      ...
      modal.innerHTML = `
        ...
        <div class="modal-content">
          <h3><iconify-icon icon="solar:sort-horizontal-bold" width="20" height="20"></iconify-icon> Quick Stats</h3>
