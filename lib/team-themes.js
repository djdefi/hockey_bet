...
(function () {
  ...
  function applyTheme(teamKey) {
    ...
    if (meta) {
      meta.setAttribute('content', vars['--color-bg-primary'] || '#0b162a');
    }
    // Cache CSS variables for instant theme-init on next page load
    try {
      if (teamKey && teamKey !== 'default') {
        localStorage.setItem('nhl_fan_team_css', JSON.stringify(vars));
      } else {
        localStorage.removeItem('nhl_fan_team_css');
        // Also clear inline styles when resetting to default
        for (var j = 0; j < keys.length; j++) {
          root.style.removeProperty(keys[j]);
        }
      }
    } catch(e) { /* noop */ }
  }
